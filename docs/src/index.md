...
`ca` / `sparse_ca` / `bincoo_ca` are available as Julia API and command line tools ([OnlineCA.jl (Julia API)](@ref), [OnlineCA.jl (Command line tool)](@ref)). `mca` is available as Julia API only.
